...
- **Primario**: usuario final que necesita resolver el caso de uso de ManosAbiertas.
...
### F1 -- Open edX
- Given el usuario en el contexto de ManosAbiertas / When usa Open edX / Then obtiene el resultado esperado sin error.
- Given entrada invalida / When la envia / Then recibe un error generico y el detalle queda en logs.

### F2 -- Moodle
- Given el usuario en el contexto de ManosAbiertas / When usa Moodle / Then obtiene el resultado esperado sin error.
- Given entrada invalida / When la envia / Then recibe un error generico y el detalle queda en logs.

### F3 -- Kolibri
- Given el usuario en el contexto de ManosAbiertas / When usa Kolibri / Then obtiene el resultado esperado sin error.
- Given entrada invalida / When la envia / Then recibe un error generico y el detalle queda en logs.

### F4 -- Oppia
- Given el usuario en el contexto de ManosAbiertas / When usa Oppia / Then obtiene el resultado esperado sin error.
- Given entrada invalida / When la envia / Then recibe un error generico y el detalle queda en logs.
